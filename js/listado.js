...
Listado.prototype.elementoFiltrado = function (data) {
    
        var filtrado = data.filter(function(elem, index, self) {
...
Listado.prototype.obtenerUbicacion = function() {
    //Array donde se van a ir agregando las ciudades (van a estar repetidas)
        // var ciudadesMezcladas = [];
        let ciudadesMezcladas = this.restaurantes.map(restaurante => restaurante.ubicacion);
    //Se recorre el array de restaurantes y se va agregando al array creado, todas las ubicaciones o ciudades encontradas
    //for (var i = 0; i < this.restaurantes.length; i++) {
       //  ciudadesMezcladas.push(this.restaurantes[i].ubicacion);
    //}
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ciudades = this.elementoFiltrado(ciudadesMezcladas);
...
Listado.prototype.obtenerRubros = function() {
    var categoriasMezcladas = this.restaurantes.map(data => data.rubro);
    /*for (var i = 0; i < this.restaurantes.length; i++) {
        categoriasMezcladas.push(this.restaurantes[i].rubro);
    }
    */
    var categorias = this.elementoFiltrado(categoriasMezcladas);
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var horariosACargados = this.restaurantes.map(data => data.horarios);
    //Recorremos el array de restaurantes y vamos agregando todos los array de horarios
    /*for (var i = 0; i < this.restaurantes.length; i++) {
        horariosACargados.push(this.restaurantes[i].horarios);
    }
    */
    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horario = [];
    horariosACargados.forEach(function(indice) {
        indice.forEach(function(hora) {
            horario.push(hora)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosCargados = this.elementoFiltrado(horariosACargados);

    return horariosCargados.sort();
}
