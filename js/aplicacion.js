...
Aplicacion.prototype.calificarRestaurant = function (restaurant) {
  ...
  swal("Ingrese su calificación (valor numérico entre 1 y 10) :", {
    ...
    if (nuevaCalificacion >= 1 && nuevaCalificacion <= 10) {
      ...
      restaurantActualizar
        .find(".puntuacion")
        .html(restaurant.obtenerPuntuacion());
    } else {
      swal({
        title: "Error",
...
Aplicacion.prototype.reservarUnHorario = function (restaurant, horario) {
  ...
  var horarioASacar = restaurantActualizar.find(
    "span:contains(" + horario + ")"
  );
  ...
  if (cantidadHorarios === 1) {
    restaurantActualizar
      .find(".reserva")
      .html("No hay más mesas disponibles 😪");
  }
  ...
  swal({
    ...
    text:
      "Has reservado una mesa en " + restaurant.nombre + " a las " + horario,
    icon: "success",
    button: "Continuar",
  });
};
